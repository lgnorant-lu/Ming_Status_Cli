  ...
  group('配置系统最终集成测试', () {
    ...
    group('端到端配置工作流测试', () {
      test('完整配置生命周期应正常工作', () async {
        ...
        // 4. 配置继承测试
        print('📋 4. 配置继承测试');
        final configWithInheritance = await configManager.loadWorkspaceConfigWithInheritance();
        ...
        final mergedConfig = config.mergeWith(updatedConfig, strategy: ConfigMergeStrategy.merge);
      ...
      test('用户配置与工作空间配置集成', () async {
        ...
        final saveSuccess = await userConfigManager.saveUserConfig(updatedUserConfig);
        ...
        print('✅ 用户配置与工作空间配置集成测试通过');
      });
    });

    group('ConfigCommand命令集成测试', () {
      test('配置命令完整功能测试', () async {
        print('📋 ConfigCommand集成测试');
        ...
        // 3. 测试配置设置功能
        final updateSuccess = await configManager.updateWorkspaceConfig((config) {
        ...
        print('✅ ConfigCommand集成测试通过');
      });
    });

    group('DoctorCommand配置检查集成测试', () {
      test('doctor命令配置深度检查集成', () async {
        print('📋 DoctorCommand配置深度检查集成测试');

        // 1. 测试配置专用检查器
        final configCheckers = doctorCommand.getCheckersForTest(configOnly: true);
        expect(configCheckers.length, equals(4));
        
        // 2. 逐个执行配置检查器
        for (final checker in configCheckers) {
          print('  🔍 执行检查器: ${checker.name}');
          ...
          if (hasErrors) {
            print('    ⚠️  检查器 ${checker.name} 发现错误: ${result.errors.length}个');
            for (final error in result.errors) {
              print('      ❌ ${error.message}');
            }
          } else {
            print('    ✅ 检查器 ${checker.name} 通过');
          }
        }

        // 3. 测试自动修复功能
        final fixableCheckers = configCheckers.where((c) => c.canAutoFix).toList();
        print('  🛠️  可自动修复的检查器: ${fixableCheckers.length}个');
        
        for (final checker in fixableCheckers) {
          try {
            final fixResult = await checker.autoFix();
            print('    ${fixResult ? '✅' : '⚠️'} ${checker.name} 自动修复: ${fixResult ? '成功' : '未修复'}');
          } catch (e) {
            print('    ❌ ${checker.name} 自动修复异常: $e');
          }
        }

        print('✅ DoctorCommand配置深度检查集成测试通过');
      });
    });
    ...
    group('错误恢复与边界条件测试', () {
      test('配置文件损坏的恢复能力', () async {
        ...
        try {
          ...
          print('  📋 配置重建结果: ${rebuildSuccess ? '成功' : '跳过(文件存在)'}');
        } catch (e) {
          print('  ⚠️  配置重建遇到预期问题: $e');
        }
...
/// 设置完整的测试环境
Future<void> _setupCompleteTestEnvironment(Directory tempDir, ConfigManager configManager) async {
  // 创建完整的目录结构
