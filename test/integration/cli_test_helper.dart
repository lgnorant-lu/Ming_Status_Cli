...
class CliTestResult {
  ...
  /// 是否包含指定文本
  bool containsOutput(String text) => stdout.contains(text) || stderr.contains(text);
...
class CliTestHelper {
  ...
  static Future<CliTestResult> runCommand(
    ...
    try {
      ...
      await Future.wait([
        process.stdout.transform(const SystemEncoding().decoder).forEach(stdoutBuffer.write),
        process.stderr.transform(const SystemEncoding().decoder).forEach(stderrBuffer.write),
      ]);
  ...
  static void expectOutput(CliTestResult result, String expectedText) {
    expect(result.containsOutput(expectedText), isTrue, 
  ...
  static void expectDuration(CliTestResult result, Duration maxDuration) {
    expect(result.duration, lessThan(maxDuration),
