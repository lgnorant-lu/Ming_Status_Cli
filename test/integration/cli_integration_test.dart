  ...
  group('CLI集成测试', () {
    ...
    group('命令测试', () {
      ...
      test('version --detailed应该显示详细信息', () async {
        final result = await CliTestHelper.runCommand(['version', '--detailed']);
      ...
      test('doctor命令应该工作正常', () async {
        ...
        CliTestHelper.expectSuccess(result);
        CliTestHelper.expectOutput(result, 'Ming Status CLI');
        CliTestHelper.expectOutput(result, '(4/4)');
      });
    ...
    group('错误处理', () {
      test('应该处理无效参数', () async {
        final result = await CliTestHelper.runCommand(['version', '--invalid-flag']);
        
        CliTestHelper.expectFailure(result);
      });

      test('应该提供有用的错误信息', () async {
        final result = await CliTestHelper.runCommand(['init', '--invalid-option']);
