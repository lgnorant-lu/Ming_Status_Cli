  ...
  group('简化压力测试', () {
    ...
    group('并发性能测试', () {
      ...
      test('应该能够处理连续操作', () async {
        ...
        final averageTime = operationTimes
            ...
            .reduce((a, b) => a + b) / operationCount;
        
        final maxTime = operationTimes
            .map((d) => d.inMilliseconds)
            .reduce(math.max);
    ...
    group('内存压力测试', () {
      ...
      test('应该能够监控内存使用趋势', () async {
        ...
        expect(snapshots.every((s) => s.containsKey('usage_percentage')), isTrue);
...
Future<void> _simulateOperation() async {
  ...
  // 模拟I/O操作
  await Future<void>.delayed(Duration(milliseconds: math.Random().nextInt(10) + 5));
