...
class PerformanceMonitor {
  ...
  Future<T> measureExecution<T>(
    ...
    try {
      ...
      // 记录内存使用变化
      if (memoryDelta.abs() > 1024 * 1024) { // 超过1MB变化才记录
        recordMetric(PerformanceMetric(
  ...
  /// 计算类型评分
  double _calculateTypeScore(PerformanceMetricType type, List<PerformanceMetric> metrics) {
  ...
  /// 获取瓶颈描述
  String _getBottleneckDescription(PerformanceMetricType type, List<PerformanceMetric> metrics) {
    switch (type) {
      case PerformanceMetricType.memory:
        final maxMemory = metrics.map((m) => m.value).reduce((a, b) => a > b ? a : b);
        return '内存使用过高: ${_formatBytes(maxMemory.toInt())}';
      case PerformanceMetricType.execution:
        final maxTime = metrics.map((m) => m.value).reduce((a, b) => a > b ? a : b);
  ...
  /// 获取优化建议
  List<String> _getRecommendations(PerformanceMetricType type, List<PerformanceMetric> metrics) {
    switch (type) {
      case PerformanceMetricType.memory:
        return [
          '考虑使用对象池减少内存分配',
  ...
  Map<String, dynamic> getPerformanceStats() {
    ...
    return {
      ...
      'baseline_memory': _baselineMemory != null ? _formatBytes(_baselineMemory!) : 'unknown',
