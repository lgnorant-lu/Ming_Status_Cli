...
class InheritanceValidator {
  ...
  Future<InheritanceValidationResult> validateInheritance(
    ...
    try {
      ...
      if (enablePerformanceCheck) {
        final performanceIssues = await _validatePerformanceImpact(inheritanceChain);
  ...
  Future<List<ValidationIssue>> _validateChainIntegrity(
    ...
    if (inheritanceChain.length > maxChainDepth) {
      issues.add(ValidationIssue(
        ...
        details: {'actualDepth': inheritanceChain.length, 'maxDepth': maxChainDepth},
      ),);
    }
  ...
  /// 验证性能影响
  Future<({List<ValidationIssue> issues, List<String> recommendations})> _validatePerformanceImpact(
    List<InheritanceNode> inheritanceChain,
  ) async {
    final issues = <ValidationIssue>[];
    final recommendations = <String>[];
  ...
  /// 验证最佳实践
  Future<({List<ValidationIssue> issues, List<String> recommendations})> _validateBestPractices(
    List<InheritanceNode> inheritanceChain,
  ) async {
    final issues = <ValidationIssue>[];
    final recommendations = <String>[];
  ...
  Map<String, dynamic> _generateValidationSummary(
    ...
    summary['fatalErrors'] = issues.where((i) => i.severity == ValidationSeverity.fatal).length;
    summary['errors'] = issues.where((i) => i.severity == ValidationSeverity.error).length;
    summary['warnings'] = issues.where((i) => i.severity == ValidationSeverity.warning).length;
    summary['infos'] = issues.where((i) => i.severity == ValidationSeverity.info).length;
    summary['maxDepth'] = inheritanceChain.isNotEmpty ? inheritanceChain.map((n) => n.depth).reduce(max) : 0;
