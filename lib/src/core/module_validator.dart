...
class ModuleValidator {
  ...
  /// 验证基础目录结构
  Future<void> _validateBasicStructure(String modulePath, ValidationResult result) async {
    // 检查模块目录是否存在
    ...
    for (final item in requiredItems) {
      ...
      if (isDirectory) {
        if (!FileUtils.directoryExists(itemPath.substring(0, itemPath.length - 1))) {
  ...
  /// 验证配置文件
  Future<void> _validateConfiguration(String modulePath, ValidationResult result) async {
    // 验证module.yaml
  ...
  /// 验证模块配置文件
  Future<void> _validateModuleConfig(String modulePath, ValidationResult result) async {
    ...
    try {
      ...
      if (yamlData.containsKey('id')) {
        ...
        if (!StringUtils.isValidIdentifier(moduleId)) {
          ...
          result.addWarning('建议使用完整的包名格式: com.company.module_name', file: configPath);
        }
      }
  ...
  /// 验证pubspec.yaml配置
  Future<void> _validatePubspecConfig(String modulePath, ValidationResult result) async {
  ...
  /// 验证代码结构
  Future<void> _validateCodeStructure(String modulePath, ValidationResult result) async {
    ...
    // 验证主模块文件
    final moduleFiles = dartFiles.where((file) => 
  ...
  /// 验证依赖关系
  Future<void> _validateDependencies(String modulePath, ValidationResult result) async {
    final pubspecPath = path.join(modulePath, 'pubspec.yaml');
    ...
    try {
      ...
      // 检查是否有依赖
      if (yamlData.containsKey('dependencies') && yamlData['dependencies'] is Map) {
      ...
      // 检查开发依赖
      if (yamlData.containsKey('dev_dependencies') && yamlData['dev_dependencies'] is Map) {
  ...
  /// 验证命名规范
  Future<void> _validateNamingConventions(String modulePath, ValidationResult result) async {
    // 验证模块目录名
  ...
  /// 验证文件内容命名
  Future<void> _validateFileNaming(String filePath, ValidationResult result) async {
    try {
      final content = await FileUtils.readFileAsString(filePath);
      if (content == null) return;
      
      // 查找类定义
      final classRegex = RegExp(r'class\s+([A-Za-z_][A-Za-z0-9_]*)', multiLine: true);
  ...
  Future<bool> quickValidate(String modulePath) async {
    try {
      ...
      if (!FileUtils.fileExists(path.join(modulePath, 'module.yaml'))) return false;
      if (!FileUtils.fileExists(path.join(modulePath, 'pubspec.yaml'))) return false;
      if (!FileUtils.directoryExists(path.join(modulePath, 'lib'))) return false;
