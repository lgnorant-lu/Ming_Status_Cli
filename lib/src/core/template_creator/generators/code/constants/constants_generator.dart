...
class ConstantsGenerator extends BaseCodeGenerator {
  ...
  String generateContent(ScaffoldConfig config) {
    ...
    // 生成主要的Constants类
    _generateMainConstantsClass(buffer, config);
  ...
  List<String> _getImports(ScaffoldConfig config) {
    ...
    // Flutter material import removed as it's not used in constants
  ...
  void _generateAppConstants(StringBuffer buffer, ScaffoldConfig config) {
    ...
    buffer.writeln('  /// 应用名称');
    buffer.writeln(
        "  static const String appName = '${_getCapitalizedName(config)}';");
    buffer.writeln();
    ...
    buffer.writeln('  /// 应用包名');
    buffer.writeln(
        "  static const String packageName = 'com.example.${config.templateName}';");
    buffer.writeln();
  ...
  void _generateApiConstants(StringBuffer buffer, ScaffoldConfig config) {
    ...
    buffer
        .writeln('  static const String baseUrl = baseUrlDev; // TODO: 根据环境配置');
    buffer.writeln();
    ...
    buffer.writeln(
        "  String get ${config.templateName}s => '/api/v1/${config.templateName}s';");
    buffer.writeln(
        "  String get ${config.templateName}ById => '/api/v1/${config.templateName}s/{id}';");
    buffer.writeln();
    ...
    buffer.writeln(
        "  String get defaultUserAgent => '${_getCapitalizedName(config)}/1.0.0';");
    buffer.writeln();
  ...
  void _generateUiConstants(StringBuffer buffer, ScaffoldConfig config) {
    ...
    buffer
        .writeln('  Duration get medium => const Duration(milliseconds: 300);');
  ...
  void _generateConfigConstants(StringBuffer buffer, ScaffoldConfig config) {
    ...
    buffer
        .writeln("  String get format => '[{timestamp}] {level}: {message}';");
  ...
  void _generateErrorConstants(StringBuffer buffer, ScaffoldConfig config) {
    ...
    buffer.writeln('  /// 错误消息');
    buffer
        .writeln('  static const ErrorMessages messages = ErrorMessages._();');
    buffer.writeln();
    ...
    buffer
        .writeln("  String get authenticationError => 'AUTHENTICATION_ERROR';");
  ...
  /// 生成主要的Constants类
  void _generateMainConstantsClass(StringBuffer buffer, ScaffoldConfig config) {
    final className = formatClassName(config.templateName, 'Constants');

    buffer.write(
      generateClassDocumentation(
        className,
        '${config.templateName}主要常量类',
        examples: [
          '$className.appName',
          '$className.appVersion',
          '$className.defaultTimeout',
          '$className.maxRetryAttempts',
        ],
      ),
    );
    ...
    buffer.writeln('  /// 应用名称');
    buffer.writeln(
        '  static const String appName = \'${formatClassName(config.templateName, '')}\';');
    buffer.writeln();

    buffer.writeln('  /// 应用版本');
    buffer.writeln('  static const String appVersion = \'1.0.0\';');
    buffer.writeln();

    // 配置常量
    buffer.writeln('  /// 默认超时时间');
    buffer.writeln(
    ...
    buffer.writeln('  /// 默认页面大小');
    buffer.writeln('  static const int defaultPageSize = 20;');
    buffer.writeln();

    // 国际化
    buffer.writeln('  /// 支持的语言');
    buffer.writeln(
        '  static const List<String> supportedLanguages = [\'en\', \'zh\'];');
    buffer.writeln();

    buffer.writeln('  /// 默认语言');
    buffer.writeln('  static const String defaultLanguage = \'en\';');
    buffer.writeln();
