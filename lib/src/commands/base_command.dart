...
import '../core/module_validator.dart';
...
abstract class BaseCommand extends Command<int> {
  ...
  Future<WorkspaceConfig?> getWorkspaceConfig() async {
    ...
    return await configManager.loadWorkspaceConfig();
  }
  ...
  /// 获取用户输入
  String? getUserInput(String prompt, {String? defaultValue, bool required = false}) {
    ...
    if (input == null || input.isEmpty) {
      if (required && defaultValue == null) {
        Logger.error('此项为必填项');
        return getUserInput(prompt, defaultValue: defaultValue, required: required);
      }
      return defaultValue;
    }
  ...
  /// 验证模块路径
  Future<bool> validateModulePath(String modulePath, {bool showDetails = false}) async {
